...
## 4. linear — Manage Linear issues via MCP
...
Skip ingestion as-is (defer). Native Linear MCP access already covers the capability, and the setup half of the skill is Codex-only. The repository has since vendored an MCP-first Linear workflow skill at `.kiro/skills/linear/SKILL.md` (adapted from `wrsmith108/linear-claude-skill`), which already provides the guardrails and workflow structure this skill would have contributed — further reducing any reason to ingest it.
