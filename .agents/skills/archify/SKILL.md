...
> **Provenance:** Vendored from [tt-a1i/archify](https://github.com/tt-a1i/archify) at tag
> `v2.16.0` (commit `c826e6c3a7abad19c0f3cd1ca57207d54b1ad8de`, MIT). Placed in
> `.agents/skills/` so Antigravity (workspace scope), Codex CLI, and opencode discover it.
> Unmodified except for this note and two trims that upstream's runtime never reads: the
> upstream `test/` directory and the pre-rendered `examples/*.html` showcases (the
> `examples/*.json` sources this skill's authoring path requires are all retained).
> On restricted machines set `ARCHIFY_UPDATE_CHECK_DISABLED=1` to disable the optional
> update-reminder GET described under "Update awareness". Intake record:
> `docs/ai-resource-intake.md` entry 4 (Linear COL-347).
