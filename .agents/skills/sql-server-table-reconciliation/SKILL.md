...
> **Provenance:** Vendored from [github/awesome-copilot](https://github.com/github/awesome-copilot/tree/4742f265959bf025882314564b364d9d7af6e2d5/skills/sql-server-table-reconciliation) (MIT licensed) at commit `4742f26`. Unmodified except for this note.
>
> **Known gaps in the bundled script** — see `scripts/reconcile.py`. `--chunk-size` is accepted but not applied, the hash pre-check is defined but never called, `--output parquet` is documented but not implemented, and the type-normalization rules below are not enforced in code. Treat the "Performance Considerations" table as design intent, not current behavior.
