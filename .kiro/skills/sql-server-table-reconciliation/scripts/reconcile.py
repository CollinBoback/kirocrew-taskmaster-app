...
Vendored from github/awesome-copilot (MIT) @ 4742f26:
https://github.com/github/awesome-copilot/tree/4742f265959bf025882314564b364d9d7af6e2d5/skills/sql-server-table-reconciliation

KNOWN GAPS (not yet fixed -- see SKILL.md):
  - --chunk-size is parsed and passed but never applied; extract_table() does a
    single full SELECT regardless of table size.
  - extract_hashes() implements the documented >1M-row hash pre-check but is
    never called.
  - --output parquet is documented in SKILL.md but absent from argparse choices.
  - No type normalization (string trim / decimal scale / datetime UTC) despite
    SKILL.md requiring it -- expect false positives on CHAR-padded columns.
  - schema_drift is reported but does not affect PASS/FAIL status.
  - Table and PK identifiers are f-string interpolated into SQL, which the
    SKILL.md constraints explicitly forbid. Needs bracket-quoting/validation.
  - TrustServerCertificate=yes is hardcoded, disabling certificate validation.
  - No connection retry logic and connections are never closed.
